...
#define RG_BACKLIGHT      SDLK_3
    ...
    void TProfanationGame::updateInputController()
    {
        ...
        inputController.setStatus(GKEY_LONG_JUMP, KeyState[RG_Y]);
        inputController.setStatus(GKEY_SHORT_JUMP,KeyState[RG_B]);
    
        inputController.setStatus(GKEY_SELECT,KeyState[SDLK_RETURN]);
        inputController.setStatus(GKEY_ABORT,KeyState[SDLK_ESCAPE]);

        inputController.setStatus(GKEY_VOLUME_UP,KeyState[RG_R]);
        inputController.setStatus(GKEY_VOLUME_DOWN,KeyState[RG_L]);

        inputController.setStatus(GKEY_SCR_PLUS,KeyState[RG_X]);
        inputController.setStatus(GKEY_SCR_MINUS,KeyState[RG_A]);
    };
